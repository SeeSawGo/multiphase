         ...
         Box3D const& imageDomain, Box3D const& vtkDomain, std::string fname, Array<T,3> location, T dx, T dt )
{
    ...
    vtkOut.writeData<3,float>(*boundaryCondition.computeVelocity(vtkDomain), "u", dx/dt);
         ...
         OffLatticeBoundaryCondition3D<T,DESCRIPTOR,Velocity>& boundaryCondition, plint level, Array<T,3> location, T dx, T dt )
{
    ...
    Array<T,3> yz_plane(0.015, 0.032604, 0.057772);
    Array<T,3> xz_plane(0.026725, 0.0005, 0.057772);
    Array<T,3> xy_plane(0.026725, 0.032604, 0.0005);
    ...
    while(!velocityTracer.hasConverged() && currentTime<simTime)
    {
        if (i%200==0 && performOutput) {
            pcout << "T= " << currentTime << "; "
                  ...
                  << "RMS Vorticity: "
                  << boundaryCondition.computeRMSvorticity()/dt << std::endl;
